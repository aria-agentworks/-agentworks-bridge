...
```
       ARIA VERSE OVERVIEW  
   ────────────────────────────
   World 1: Mars  (Colony)   │ World 2: Earth  (Future 2147) │ World 3: Origin  (65M YA)     
    ──────────────────────────────────────────────────────────────────────────────
...
| Earth Origin | Reach Mars Level 25 | 35-45 | 120,000+ | Primordial survival begins |

**Progression Carry-Over:**

| Static | Carried Over? | Transfer Method | Max Cap |
|-------|-----------|---------------- |---------|
...
```
    ...
    ────────────────────────────
├─ Magical-equipped boots (base movement)       
...
```
    ...
    ───────────────────────────────────────
├─ Primary weapon set (stone spear, club)       
├─ Hunger/thirst mechanics tutorial       
├─ Tribal affiliation access (any of 5)       
├─ Boss Drove spawn awareness       
├─ Access to Primal Zone       
...
| Future Earth | Trading, contracts, raching | Cybernetics, properties, vehicles | 1 AC = 1 AC |
...
```
   ────────────────────────────
    ...
    ────────────────────────────
    MARSTOWN HUB (World 1)   │ NEO-VERIDIA (World 2)   │ ORIGIN BANK (World 3)
       (Starting World)       │ (Unlock @ 2500XP)       │ (Unlock @ 12000XP)
    ──────────────────────────────────────────────────────────────────────
    Mars → Future Earth │ Mars → Earth Origin │ Future Earth → Mars │ Earth Origin → Mars
       1:1 exchange        1:1 exchange        1:1 exchange        1:1 (2% fee)
    ──────────────────────────────────────────────────────────────────────
       ARIA PORTAL HUB (Universe Exchange)
     ─────────────────────────────────────
     (Multi-World Trade Hub)
       Mars 1:1 exchange
       Earth Origin: 1:1 (2% fee)
       Future Earth: 1:1
     ─────────────────────────────────────
     ─────────────────────────────────────
       │ PREMIUM ZONE (World 3) │ VERSE BANK         │
       │ Boss Drops          │ (Verse Hub)  │ BANK         │
...
```
       PORTAL HUB INTERFACE       
    ────────────────────────────
   Current Location: MARSTOWN Hub     │ NEO-VERIDIA   │ PRIMAL ZONE       
        (World 1)                  │ (Future)      │ (Origin)        
        │─────────────────────────────────────────────────────┐
    TRAVEL LOG:                     
    ├─ Visit: Neo-Veridia (2 hours ago)    
    ├─ Visit: Primal Zone (yesterday)     
    │──────────────────────────────────┐
    [RECALL HOME] [BROWSE] [CLOSE]    
    ───────────────────────────────────┐
    ─────────────────────────────────────
```
...
|----------|-------------------|---------|----------|
| World Explorer | 15 | All-purpose skins | "Mars Native", "Neo-Veteran", "Primal Survivor" |
...
| Time Traveler | 5 | Temporary cosmetics | "Chronos Badge", "Era霍珀" |
...
# ## 10. WORLD 2 - FUTURE EARTH (CYBERPUNK PARLOURK)
...
- **Atmosphere:** Cybepunk dystopia with neon-lit sprawl, corporate dominance
...
| Neon Snickers | Movement slow | 24h | Antidote injection |
...
```
    ...
    ────────────────────────────
   NAVIGATE    │ TRADE    │ TERRITORY        
    │    │    │    │    │    │    │
Parkour routes   Contracts Tools     Areas     Territorial         
    │    │    │    │    │    │    │
Earn Credits Buy Properties Control  Terriority         
    │    │    │    │    │    │    │
Unlock Routes Complete Missions Upgrade Gear Expand     
    ────────────────────────────
```
...
function handleParkourAction(actionType, player)
    ...
    elseif actionType == "grapple_fire" then
    ...
    if success then
        setCooldown(player, actionType, 2.0)
...
| Mag-boots climb speed | 15 stud/s | 25 stud/s | 35 stud/s |
...
```
   ...
   RISKS: Capture = lose half loot, permanent debuff       
   HEIST PROCEDURES:                   
...
|-------------|-----------|----------|------------------|-----------------|
| **NeCorp** | Neutral | Tech upgrades, cybernetics | Corporate espionage, data theft | +10 per contract |
| **AeroDyn** | Good | Vehicles, transportation | Delivery, rescue missions | +5 per contract |
...
| Data Storm | Random (2hr) | Random debuffs | Firewall shield |
...
|-----------|-------|----------|--------|
| Cyber Skins | 500 - 5,000 AC | Neo armor, LEE enhancements, plasma implants | Common-Legendary |
...
- **Atmosphere:** Primordial, raw survival, no technology
...
| Alpha Territory | 10% | Extreme | Primordial Essence, rare resources | Alpha Dragon Dron |

### 11.2 Core Loop: Primordial Hunting + Boss Raids + Resource Gathering
...
```
   ...
   GATHER    │ HUNT    │ RAID       
    │    │    │    │    │    │    │
Gatherings resources Craft Weapons Hunt Bosses Escape         
    │    │    │    │    │    │    │
Tools to  │   │ AI Guardians     │ Boss Loot         
    │    │    │    │    │    │    │
...
3. **Boss Dron Raids (30% of gameplay)**
...
| Alpha Dragon Dron | Alpha Territory | 5,000 HP | 24 hr cooldown | Aria Primordial Set | 35 |
...
function initializeBossDrone(bossType, location)
    local bossProperties = {
        ["IceGuardian"] = {
            hp = 1000,
            damage = 50,
            abilities = {"freeze", "ice_spikes", "avalance"},
        ...
        ["ScorchGuardian"] = {
            hp = 2000,
            damage = 75,
            abilities = {"lava_burst", "heat_wave", "mgma_spawn"},
        ...
        ["AlphaDragon"] = {
            ...
            resistances = {all = 0.5},
            drops = {"aria_primordial_set", "dragon_heart", "legendary_trophy"}
        }
    }
...
### 11.4 Primordial Mechanics

**Stone Weapon Crafting:**
...
| Tribe | Specialization | Bonuses | Leader Quest | Member Benefits |
|-------|----------------|---------|--------------|-----------------|
| Huntmasters | Hunting, combat | +20% damage vs creatures | Defeat 50 creatures | Rare weaponblueprints |
| Gatherers | Resource gathering | +25% resource yield | Collect 1000 resources | Auto-collector tool |
| Builders | Structure building | +15% building speed | Build structures | Advanced hammer |
...
```
   ...
   Level 10: Adepts - Exclusive items       
   Level 20: Master - Tribal hall privleges       
...
|----------|----------|------|--------|--------|
| Stone | 100+ | Crafting, building | Common | Everywhere |
| Wood | 200+ | Crafting, fire | Common | Forest areas |
| Bone | 30+ | Weapon crafting | Uncommon | Creature remains |
| Hide | 500+ | Clothing, crafting | Uncommon | Hunt rewards |
| Herbs | 800+ | Healing, potions | Common | Jungle, meadows |
| Minerals | 20+ | Advanced crafting | Rare | Cave systems |
...
| Premium Weapons | 1,000 - 5,000 AC | Diamond spear, Adamant axe, Obsidian blade | +20% damage, unlimited durability |
| Tribal Hall Decorations | 500 - 10,000 AC | Statues, carpets, banners | +2% tribal reputation |
...
| Bundle | Price | Contents | Value |
|--------|------|----------|-------|
| Primordial Starter | 1,000 AC | Basic gear, 500 AC, weapon | 3,000 AC |
| Tribal Champion | 3,000 AC | Premium weapons, hall decor, 5 tickets | 10,000 AC |
| Boss Slayer | 10,000 AC | Full primordials set, 20 tickets | 30,000 AC |
...
```
    ...
    ─────────────────────────────
    Agent 1: MapGen │ Agent 2: Behavior │ Agent3: Econo │ Agent 4: AntiChurn    
```
...
function MapGenAI:updateMap(heartbeat)
    ...
    if currentTime - self.lastUpdate >= 1800 then
...
function PlayerPredictionAgent:deployTargetedRewards(player, rewardConfig)
    local currentTime = os.time()
...
function EconomyAgent:getEconomyMetrics()
    ...
    local totalAC = self:getTotalAriaCreditsInCirculation()
    local earnedPerHour = self:getACGeneratedPerHour()
    local spentPerHour = self:getACConsumedPerHour()
...
function AntiChurnAgent:assessChurnRisk(player)
    ...
    -- Calculate churn score via LLM model
    local prompt = string.format([
...
```
   ...
   PREMIUM PAYOUT OPTIMALITY: 2-3 purchases per session    
   ──────────────────────────────────────
```
...
### 13.1 Unified Currency with Exchange Rates
...
```
   ...
   CROSS-WORLD ECONOMY                    
    ───────────────────────────────────────
  MARSTOWN (World 1)        │ NEO-VERIDIA (World 2)    
       │                    │                       
 ...
 Future Earth        │ Mars        
   │ 1:1 exchange (2% fee)        
       │                    │                    
 EARTH ORIGIN (World 3)       
       │                    │                    
 Boss drop sales        │                
   Tribal crafting        │                
  Resource gathering      │                
       │                    │                    
 PRIMORDIAL ZONE (World 3) │ VERSE BANK    
   │                    │ (Verse Hub)     
 1:1 (2% fee)         │ Interest: 5%/year    
```
...
| Co-op Play | Up to 10 players | Cross-world dungeons |
...
| Item Type | Mars → Earth | Mars ← Earth | Earth → Mars | Origin → Mars |
|-----------|--------------|--------------|--------------|---------------|
| Buildings | X | X | X | X |
| Vehicles | X | X | ✓ | X |
| Weapons | ✓ | ✓ | ✓ | ✓ |
...
├─ Temporary weapon (time manipulation ability)      
├─ 50,000 Aria Credits      
...
### B.2 ARIA-GHOST Hub Technical Spec
...
```
   ...
   └─ Claude-3: Economics Balancing (complex analysis)      
    │    
    Agent Functions:      
...
| **Theme** | Colony building | Cyberpunk parkour | Primordial survival |
...
| Aria Credits universal system | P0 | Medium | High | Months 1-2 |
...
Join our Discord: discord.gg/averase
